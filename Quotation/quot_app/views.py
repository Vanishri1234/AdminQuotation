...
def save_purchase_details(request):
    if request.method == 'POST':
        ...
        try:
            ...
            last_quot = Quot.objects.all().order_by('quot_Number').last()
            if last_quot:
                last_number = int(last_quot.quot_Number)
                quot_number = f"{last_number + 1:05d}"
            else:
                quot_number = "0001"
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Company.objects.filter(companyName=company_name).exists():
                form.add_error('companyName', 'A company with this name already exists.')
            else:
                form.save()
                return redirect('company_list')  # Redirect to a company list page or any other page
    else:
        form = CompanyForm()
